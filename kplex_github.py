# KPLEX CODE TAKEN FROM https://github.com/bachsh/kplex/blob/master/kplex.py
# Only minor modifications done
...
# 10:   Compute the intersection C of the neighborhoods of the n critical vertices and Remove all the vertices in candidate and not which are not in C;
    criticalNodes = list(filter(lambda node: compsubCount[node] == k-1, compsub))
